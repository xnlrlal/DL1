'''
    1. 파이썬 리스트 & 넘파이
        ...
        5) 넘파이는 강력한 배열 프로그래밍 기능으로 넘파이가 인기있는 이유임.
            - 스칼라(scalar) 언어의 for문
...
print(list_sum)     # 리스트를 이루는 원소들의 인덱스가 특별한 의미를 갖지 않음(단순히 리스트 내의 순서만 나타냄)

list_sum = []
for i in range(0, 3):
    c = store_a[i] + store_b[i]
    list_sum.append(c)
print(list_sum)
